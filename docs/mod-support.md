...
| Reduce Save Compression v1.2 | Complete one-component catalog; directly patches game executable bytes | Executable bounded BG2EE fixture; run only on disposable game copies | **Untested** — binary-pattern install and launch smoke not yet performed; other games remain Untested | [registry](../registry/packages/reduce-save-compression.yaml) |
...
selected and 80 indexed candidates. Twenty-eight of those candidates (Sirene,
...
Business, and Reduce Save Compression) now have curated executable identities; 52 remain
