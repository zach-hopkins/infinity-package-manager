...
| Reflections of Destiny 0.9.4 | Seven TP2 selectors: five other SoD/BGEE choices, one EET-only, and selector 230 blocked after two parse-error failures; four commented-out blocks excluded | Exact source route resolves executable for remaining selectors; Caelar rewrite guarded against Road to Discovery | **Incompatible** for exact selector 230; other selections **Untested** pending clean build and menu smoke | [failure evidence](pa4-bgee-reflections-feyr-failure.md) |
