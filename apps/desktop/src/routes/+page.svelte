<script lang="ts">
	...
	async function installExperience() {
		...
		error = '';
		result = null;
		progress = { stage: 'starting', message: 'Starting your new mod experience', percent: 0 };
		try {
			...
			<main class="min-w-0 px-7 py-7 lg:px-10">
				...
				<div class="mt-7 grid grid-cols-[minmax(0,1fr)_330px] gap-7">
					<section class="space-y-5">
						...
						{#if result}<div class="rounded-xl border border-emerald-500/25 bg-emerald-50 px-4 py-4 text-sm text-emerald-900"><p class="font-semibold">Your mod experience is ready</p><p class="mt-1 text-xs text-emerald-800/75">{result.actions} installation actions finished. The completed, sealed game copy is ready to play.</p><p class="mt-2 break-all rounded-lg bg-white/60 px-3 py-2 font-mono text-[11px] text-emerald-950">{result.sealed_build}</p>{#if result.weidu_warnings.length}<button onclick={() => (showInstallWarnings = !showInstallWarnings)} class="mt-3 text-xs font-semibold text-emerald-800 underline">{showInstallWarnings ? 'Hide' : 'Show'} {result.weidu_warnings.length} WeiDU warning {result.weidu_warnings.length === 1 ? 'receipt' : 'receipts'}</button>{#if showInstallWarnings}<div class="mt-2 space-y-2 rounded-lg border border-emerald-500/20 bg-white/60 p-3 text-xs text-ink-800/75">{#each result.weidu_warnings as warning}<div><p class="font-semibold">Action {warning.action}: {warning.package}</p><pre class="mt-1 whitespace-pre-wrap font-mono text-[10px] leading-4">{warning.details}</pre><p class="mt-1 break-all text-[10px] text-ink-800/50">Saved receipt: {warning.log}</p></div>{/each}</div>{/if}{/if}</div>{/if}

						{#if busy && progress}<section class="rounded-xl border border-moss-500/25 bg-moss-500/8 p-4" aria-live="polite"><div class="flex items-baseline justify-between gap-4"><div><p class="text-xs font-semibold uppercase tracking-[0.12em] text-moss-600">Building your mod experience</p><p class="mt-1 text-sm font-semibold text-ink-950">{progress.message}</p>{#if progress.action && progress.totalActions}<p class="mt-1 text-xs text-ink-800/55">Installation action {progress.action} of {progress.totalActions}</p>{/if}</div><span class="shrink-0 text-2xl font-semibold tabular-nums text-moss-600">{progress.percent}%</span></div><div class="mt-3 h-2 overflow-hidden rounded-full bg-white/80"><div class="h-full rounded-full bg-moss-600 transition-[width] duration-500 ease-out" style:width={`${progress.percent}%`}></div></div><p class="mt-2 text-[11px] leading-4 text-ink-800/50">Some mod installers can take a while inside a single action. IEPM will keep this step visible until it finishes.</p></section>{/if}
